...
Approach: Search functionality on giphy api is based on keyword search handled by axios and used boot straps grid for search result. 
Giphy search component is  responsible for all functions hover animation of image to gig and search. There will be a list of images which are rendering in react component 
I have created a react component, where I  have taken search text box and on changing of text calling webservice api where we are searching gify using gify component and on hover of all the images I  have passed the src url of gify  file, On mouse out the we are again charging out the image url and it stops the animations
